...
# ---- bias de ubicación para Google Autocomplete ----
_LOC_BIAS = {
    "lat": None,
    "lng": None,
    "radius_m": None,
    "components": "country:es",  # limita a España; puedes hacerla dinámica si quieres
}

def set_location_bias(lat: float | None, lng: float | None, radius_m: int | None = 30000, *, components: str = "country:es"):
    """
    Guarda un sesgo geográfico para Google Places:
      - locationbias=circle:{radius}@{lat},{lng}
      - components=country:XX
    """
    _LOC_BIAS["lat"] = lat
    _LOC_BIAS["lng"] = lng
    _LOC_BIAS["radius_m"] = radius_m
    _LOC_BIAS["components"] = components
...
def provider_google_autocomplete(query: str, max_results: int = 8):
    ...
    Google Places Autocomplete:
      - types=address para forzar direcciones (retiene números mejor).
      - language=es
      - components=country:es (puedes cambiarlo)
      - locationbias si tenemos lat/lng (círculo)
    """
    ...
    try:
        params = [
            ("input", query),
            ("types", "address"),        # << clave para números
            ("language", "es"),
            ("key", GOOGLE_PLACES_API_KEY),
        ]
        # components (p.e. limitar a España)
        if _LOC_BIAS.get("components"):
            params.append(("components", _LOC_BIAS["components"]))

        # locationbias (círculo) si tenemos lat/lng
        if _LOC_BIAS.get("lat") is not None and _LOC_BIAS.get("lng") is not None and _LOC_BIAS.get("radius_m"):
            bias = f"circle:{_LOC_BIAS['radius_m']}@{_LOC_BIAS['lat']},{_LOC_BIAS['lng']}"
            params.append(("locationbias", bias))

        # construimos URL
        base = "https://maps.googleapis.com/maps/api/place/autocomplete/json"
        qstr = "&".join([f"{k}={quote_plus(str(v))}" for k, v in params])
        url = f"{base}?{qstr}"
...
def suggest_addresses(query: str, key_bucket: str):
    q = (query or "").strip()
    if len(q) < 3:       # pedimos 3+ para evitar ruido, puedes poner 2 si quieres
        return []
    # Google primero (ya con 'address' + bias); si no, SerpAPI; si no, Nominatim
