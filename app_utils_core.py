...
def _addr_from_any(x) -> t.Optional[str]:
    """Normaliza dict/str a 'lat,lng' o dirección en texto."""
    if x is None:
        return None
    if isinstance(x, str):
        s = x.strip()
...
def suggest_addresses(term: str, min_len: int = 3, max_results: int = 8) -> list[dict]:
    """Autocompletar direcciones (opcional)."""
...
def resolve_selection(term: str, place_id: str | None = None) -> dict:
    """Convierte texto/place_id a dict {address,lat,lng} robusto."""
    ...
    try:
        ...
        addr = res.get("formatted_address") or res.get("name") or term
        loc = res.get("geometry", {}).get("location", {}) if isinstance(res, dict) else {}
...
def build_gmaps_url(origin, destination, waypoints=None, mode="driving", avoid=None, optimize=True):
    """URL de Google Maps con paradas (vía /dir/?api=1)."""
    def _addr(x): return _addr_from_any(x) or None
    ...
    parts = [
        ...
        f"travelmode={(mode or 'driving').lower()}",
    ]
    if avoid:
...
def build_apple_maps_url(origin=None, destination=None):
    """URL de Apple Maps con o sin origen."""
    o = _addr_from_any(origin); d = _addr_from_any(destination)
    if not d:
        return None
